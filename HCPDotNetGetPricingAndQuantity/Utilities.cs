...

{
    ...
    {
        private static object TotalPartCountProcessedObjectLock = new object();
        private static int _totalPartCountProcessed = 0;
        public static int TotalPartCountProcessed {
            get
            {
                return _totalPartCountProcessed;
            }
            set
            {
                lock (TotalPartCountProcessedObjectLock)
                {
                    _totalPartCountProcessed = value;
                }
            }
        }

        private static object TotalPartCountWithBadUnitObjectLock = new object();
        private static int _totalPartCountWithBadUnit = 0;
        public static int TotalPartCountWithBadUnit
        {
            get
            {
                return _totalPartCountWithBadUnit;
            }
            set
            {
                lock (TotalPartCountWithBadUnitObjectLock)
                {
                    _totalPartCountWithBadUnit = value;
                }
            }
        }

        public static int TotalPartsInDatabase { get; set; }
        ...
        private static void Log(string level, string message)
        {
            ...
            Console.WriteLine($"[{level}] - [{now.ToString("yyyy-MM-dd HH:mm:ss.fff")}] {message} - [*** Total Parts Processed So Far: {TotalPartCountProcessed} of {TotalPartsInDatabase} ***]");
        }
    }
}
